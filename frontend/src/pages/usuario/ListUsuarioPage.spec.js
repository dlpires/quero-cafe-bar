jest.mock('../../services/api.js', () => ({
  api: { getUsuarios: jest.fn(), deleteUsuario: jest.fn() },
}))
...
describe('ListUsuarioPage', () => {
  ...
  const mockUsuarios = [
    { id: 1, nome: 'Admin', usuario: 'admin', perfil: 0 },
    { id: 2, nome: 'Garçom', usuario: 'garcom', perfil: 1 },
  ]
  ...
  beforeEach(() => {
    ...
    class TestListUsuarioPage extends HTMLElement {
      constructor() {
        super()
        this.items = []
        ...
        this.querySelector = jest.fn((selector) => {
          if (selector === '.list-usuario-container') return this._container
      ...
      renderItems() {
        ...
        container.innerHTML = `<ion-list>${this.items.map(u => `
          <ion-item><ion-label>${u.nome}</ion-label></ion-item>
        `).join('')}</ion-list>`
      }
    }

    if (!customElements.get('list-usuario-page')) {
      customElements.define('list-usuario-page', TestListUsuarioPage)
    }

    page = new TestListUsuarioPage()
  })

  describe('Paginação', () => {
    it('deve renderizar controles de paginação quando há múltiplas páginas', async () => {
      api.getUsuarios.mockResolvedValue(mockPaginatedResponse)
    ...
    it('deve desabilitar "Próxima" na última página', async () => {
      api.getUsuarios.mockResolvedValue({ ...mockPaginatedResponse, skip: 20 })
    ...
    it('deve ocultar botões quando há apenas 1 página', async () => {
      api.getUsuarios.mockResolvedValue({ data: mockUsuarios, total: 2, skip: 0, take: 10 })
      await page.loadPage(1)
    ...
    it('deve ocultar barra quando não há itens', async () => {
      api.getUsuarios.mockResolvedValue({ data: [], total: 0, skip: 0, take: 10 })
      await page.loadPage(1)
    ...
    it('deve mostrar skeleton durante carregamento', async () => {
      ...
      expect(page.querySelector('.list-usuario-container').innerHTML).toContain('ion-skeleton-text')
      await loadPromise
    })
    ...
    it('deve navegar entre páginas', async () => {
      api.getUsuarios.mockResolvedValue(mockPaginatedResponse)
      await page.loadPage(1)
      api.getUsuarios.mockResolvedValue({ ...mockPaginatedResponse, skip: 10 })
      await page.nextPage()
    ...
    it('deve tratar erro com toast', async () => {
      api.getUsuarios.mockRejectedValue(new Error('Falha'))
      await page.loadPage(1)
